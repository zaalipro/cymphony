defmodule CymphonyElixir.OrchestratorStatusTest do
  ...
  test "orchestrator snapshot reflects last claude update and session id" do
    ...
    assert length(snapshot_entry.log_events) == 2
    [first, second] = snapshot_entry.log_events
    assert first.event == :notification
    assert second.event == :session_started
  end

  test "orchestrator log_events ring buffer truncates at 50 entries" do
    issue_id = "issue-log-truncation"

    issue = %Issue{
      id: issue_id,
      identifier: "MT-189",
      title: "Log truncation test",
      description: "Verify log event ring buffer truncation",
      state: "In Progress",
      url: "https://example.org/issues/MT-189"
    }

    orchestrator_name = Module.concat(__MODULE__, :LogTruncationOrchestrator)
    {:ok, pid} = Orchestrator.start_link(name: orchestrator_name)
    ...
    preloaded_logs =
      for i <- 1..55 do
        %{at: DateTime.add(started_at, -i, :second), event: :old_event, message: "log #{i}"}
      end
    ...
    snapshot = GenServer.call(pid, :snapshot)
    assert %{running: [snapshot_entry]} = snapshot
    assert length(snapshot_entry.log_events) == 50
  ...
  test "orchestrator snapshot tracks claude thread totals and app-server pid" do
    ...
    snapshot = GenServer.call(pid, :snapshot)
    assert %{running: [snapshot_entry]} = snapshot
    assert snapshot_entry.agent_os_pid == "4242"
    ...
    assert completed_state.token_totals.input_tokens == 12
    assert completed_state.token_totals.output_tokens == 4
    assert completed_state.token_totals.total_tokens == 16
    assert is_integer(completed_state.token_totals.seconds_running)
  end
  ...
  test "orchestrator snapshot tracks claude token-count cumulative usage payloads" do
    ...
    assert completed_state.token_totals.input_tokens == 10
    assert completed_state.token_totals.output_tokens == 5
    assert completed_state.token_totals.total_tokens == 15
  end
  ...
  test "orchestrator restarts stalled workers with retry backoff" do
    ...
    # Add a 200ms buffer to account for OS scheduling jitter.
  ...
  test "application configures a rotating file logger handler" do
    assert {:ok, handler_config} = :logger.get_handler_config(:cymphony_disk_log)
  ...
  test "application stop renders offline status" do
    rendered =
      ExUnit.CaptureIO.capture_io(fn ->
        assert :ok = CymphonyElixir.Application.stop(:normal)
      end)
