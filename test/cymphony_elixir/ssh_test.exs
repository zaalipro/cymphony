defmodule CymphonyElixir.SSHTest do
  use ExUnit.Case, async: false

  alias CymphonyElixir.SSH

  test "run/3 keeps bracketed IPv6 host:port targets intact" do
    test_root = Path.join(System.tmp_dir!(), "cymphony-ssh-ipv6-test-#{System.unique_integer([:positive])}")
  ...
  test "run/3 leaves unbracketed IPv6-style targets unchanged" do
    test_root = Path.join(System.tmp_dir!(), "cymphony-ssh-ipv6-raw-test-#{System.unique_integer([:positive])}")
    trace_file = Path.join(test_root, "ssh.trace")
    previous_path = System.get_env("PATH")
  ...
  test "run/3 passes host:port targets through ssh -p" do
    ...
    trace = File.read!(trace_file)
    assert trace =~ "-F /tmp/cymphony-test-ssh-config"
  ...
  test "run/3 keeps the user prefix when parsing user@host:port targets" do
    test_root = Path.join(System.tmp_dir!(), "cymphony-ssh-user-test-#{System.unique_integer([:positive])}")
    trace_file = Path.join(test_root, "ssh.trace")
    previous_path = System.get_env("PATH")
  ...
  test "run/3 returns an error when ssh is unavailable" do
    test_root = Path.join(System.tmp_dir!(), "cymphony-ssh-missing-test-#{System.unique_integer([:positive])}")
    previous_path = System.get_env("PATH")
  ...
  test "start_port/3 supports line mode" do
    test_root = Path.join(System.tmp_dir!(), "cymphony-ssh-line-port-test-#{System.unique_integer([:positive])}")
    trace_file = Path.join(test_root, "ssh.trace")
    previous_path = System.get_env("PATH")
