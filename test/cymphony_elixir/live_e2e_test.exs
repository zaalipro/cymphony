defmodule CymphonyElixir.LiveE2ETest do
  use CymphonyElixir.TestSupport

  require Logger
  alias CymphonyElixir.SSH
  ...
  @team_query """
  query CymphonyLiveE2ETeam($key: String!) {
  ...
  mutation CymphonyLiveE2ECreateProject($name: String!, $teamIds: [String!]!) {
  ...
  mutation CymphonyLiveE2ECreateIssue(
    $teamId: String!
    $projectId: String!
    $title: String!
    $description: String!
  ...
  query CymphonyLiveE2EProjectStatuses {
  ...
  @issue_details_query """
  query CymphonyLiveE2EIssueDetails($id: String!) {
    issue(id: $id) {
      id
      identifier
      state {
  ...
  mutation CymphonyLiveE2ECompleteProject($id: String!, $statusId: String!, $completedAt: DateTime!) {
  ...
  defp live_prompt(project_slug) do
    """
    You are running a real Cymphony end-to-end test.
  ...
  defp run_live_issue_flow!(backend) when backend in [:local, :ssh] do
    run_id = "cymphony-live-e2e-#{backend}-#{System.unique_integer([:positive])}"
    ...
    try do
      if is_pid(orchestrator_pid) do
        assert :ok = Supervisor.terminate_child(CymphonyElixir.Supervisor, CymphonyElixir.Orchestrator)
      end
      ...
      project =
        create_project!(
          team["id"],
          "Cymphony Live E2E #{backend} #{System.unique_integer([:positive])}"
        )
      ...
      issue =
        create_issue!(
          ...
          project["id"],
          active_state["id"],
          "Cymphony live e2e #{backend} issue for #{project["name"]}"
        )
  ...
  defp restart_orchestrator_if_needed do
    if is_nil(Process.whereis(CymphonyElixir.Orchestrator)) do
      case Supervisor.restart_child(CymphonyElixir.Supervisor, CymphonyElixir.Orchestrator) do
        {:ok, _pid} -> :ok
