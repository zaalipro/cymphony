...
tracker:
  kind: linear
  project_slug: ttsql-91909d226e06
  active_states:
    - Todo
    - In Progress
    - Merging
...
polling:
  interval_ms: 5000
workspace:
  root: ~/code/cymphony-workspaces
hooks:
  after_create: |
    git clone --depth 1 git@github.com:zaalipro/ttsql.git .
...
5.  Ensure the workpad includes a compact environment stamp at the top as a code fence line:
    ...
    - Example: `devbox-01:/home/dev-user/code/cymphony-workspaces/MT-32@7bdde33bc`
...
8.  Attach PR URL to the issue (prefer attachment; use the workpad comment only if attachment is unavailable).
    - Ensure the GitHub PR has label `cymphony` (add it if missing).
...
- Required PR metadata is present (`cymphony` label).
