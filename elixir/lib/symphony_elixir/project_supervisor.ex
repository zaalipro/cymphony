defmodule SymphonyElixir.ProjectSupervisor do
  ...
  @spec via_tuple(String.t(), atom()) ::
          {:via, Registry, {SymphonyElixir.ProjectRegistry, {String.t(), atom()}}}
  def via_tuple(project_name, role) do
    {:via, Registry, {SymphonyElixir.ProjectRegistry, {project_name, role}}}
  end
  ...
  def lookup(project_name, role) do
    case Registry.lookup(SymphonyElixir.ProjectRegistry, {project_name, role}) do
      [{pid, _}] -> pid
      [] -> nil
    end
  end
  ...
  def list_project_names do
    Registry.select(SymphonyElixir.ProjectRegistry, [{{:"$1", :"$2", :_}, [], [:"$1"]}])
  ...
  def list_orchestrators do
    Registry.select(SymphonyElixir.ProjectRegistry, [{{:"$1", :"$2", :_}, [], [{{:"$1", :"$2"}}]}])
    |> Enum.flat_map(fn
