...
def is_cymphony_bot_user(user: dict[str, Any]) -> bool:
    login = user.get("login") or ""
    return login in CYMPHONY_BOT_USERS


def is_bot_user(user: dict[str, Any]) -> bool:
    login = user.get("login") or ""
    if is_cymphony_bot_user(user):
        return True
...
def is_cymphony_reply_body(body: str) -> bool:
    return body.startswith("[cymphony]")
...
def latest_cymphony_reply_by_thread(
    comments: list[dict[str, Any]],
) -> dict[int, datetime]:
    latest: dict[int, datetime] = {}
    for comment in comments:
        body = (comment.get("body") or "").strip()
        if not is_cymphony_reply_body(body):
            continue
...
def latest_cymphony_issue_reply_time(
    comments: list[dict[str, Any]],
) -> datetime | None:
...
def filter_bot_comments(
    ...
    latest_cymphony_reply = latest_cymphony_reply_by_thread(comments)
    latest_issue_ack = latest_cymphony_issue_reply_time(comments)
    bot_comments = [c for c in comments if is_cymphony_bot_user(c.get("user", {}))]
    ...
    for comment in bot_comments:
        ...
        if not is_threaded:
            ...
            if thread_root is not None:
                last_reply = latest_cymphony_reply.get(thread_root)
...
def filter_human_issue_comments(comments: list[dict[str, Any]]) -> list[dict[str, Any]]:
    latest_ack = latest_cymphony_issue_reply_time(comments)
    filtered: list[dict[str, Any]] = []
    for comment in comments:
        if is_bot_user(comment.get("user", {})):
            continue
        body = (comment.get("body") or "").strip()
        if is_cymphony_reply_body(body):
            continue
        created_time = comment_time(comment)
        if (
...
def is_blocking_review(
    ...
    if user_login in CYMPHONY_BOT_USERS:
        return state == "CHANGES_REQUESTED"
    if body.startswith("[cymphony]") or state in ("APPROVED", "DISMISSED"):
        return False
    blocking = False
...
async def wait_for_review_feedback(pr_number: int, checks_done: asyncio.Event) -> None:
    ...
    while True:
        ...
        bot_issue_comments = filter_bot_comments(issue_comments)
        bot_review_comments = filter_bot_comments(review_comments)
        ...
        if bot_comments:
            ...
            if body:
                print("Bot left comments. Address feedback before merge.")
                print(body)
                raise SystemExit(2)
...
async def watch_pr() -> None:
    ...
    review_task = asyncio.create_task(wait_for_review_feedback(pr.number, checks_done))
