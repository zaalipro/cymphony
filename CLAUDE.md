...
Server-side re-render is **change-only** and the second-by-second clock runs in the browser. Poll/pubsub cadence itself is unchanged.
...
- Acceptance: an idle dashboard ships no per-second diff, and a poll whose only movement is `generated_at` reassigns nothing but `:token_samples` (throughput window).
