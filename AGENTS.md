...
- Project-header / session / add-project / queue-card agent+model+effort is one `.spec-switcher` (`phx-hook="SpecSwitcher"` in `layouts.ex` beside `HarnessTail`, `QueueBoard`, `Combobox`, `LiveClock`). The compact trigger is `icon model effort`; the panel is an icon strip (claude/codex/agy) plus Model and Effort flyouts — no Reset. Hidden inputs keep stable ids `#agent-<name>`, `#model-<name>`, `#effort-<name>` (never embed kind/effort). `#add-project-slug` stays a Combobox. Effort row is hidden when the selected kind is `antigravity`. Changing a known kind persists immediately (kind only). Header Set and `POST /api/v1/agent` rewrite the project `WORKFLOW.md` and overlay `config.json` so `snapshot.agent_kind` survives refresh.
